...
#   name        = "mlops-terraform-state-manage"
#   description = "S3 bucket access policy for managing terraform state for mlops"
...
data "aws_iam_user" "s3_bucket_cicd_user" {
  user_name = "leone17"
}
...
resource "aws_iam_access_key" "s3_bucket_cicd_key" {
  user = data.aws_iam_user.s3_bucket_cicd_user.user_name
}

// Create a second S3 bucket and DynamoDB table for storing terraform state for CI/CD
// service principals
data "aws_s3_bucket" "cicd_terraform_state" {
  bucket = "databricks-leone"
...
resource "aws_dynamodb_table" "cicd_terraform_state_lock" {
  name           = "mlops-cicd-setup-tfstate-lock"
  read_capacity  = 1
  write_capacity = 1
  hash_key       = "LockID"

...
def _get_ml_config_value(env, key):
    ...
    try:
        ...
        raise RuntimeError(
            ...
            f"mlops have been deployed to {env} (see mlops"
...
def get_model_name(env, test_mode=False):
    ...
    if env == "dev" or test_mode:
        ...
        return f"mlops-model{resource_name_suffix}"
    else:
        # Read ml model name from mlops/terraform
        return _get_ml_config_value(env, "mlops_model_name")
